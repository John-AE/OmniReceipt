...
const Template11: React.FC<Template11Props> = ({ data }) => {
  ...
  const primaryColor = data.primaryColor || '#EF3E36'; // Default red for Template 11
      ...
      <div className="bg-white p-8 w-[794px] mx-auto">
        ...
        <table className="w-full mb-8 border border-gray-300">
          ...
          <tbody>
            ...
            {data.items.filter(item => item.isPayment).map((item, index) => (
              <tr key={`payment-${index}`} className="italic" style={{ backgroundColor: `${primaryColor}10` }}>
                <td className="p-2 border border-gray-300 font-medium" colSpan={3} style={{ color: primaryColor }}>
                  {item.description}
                </td>
                <td className="p-2 text-right border border-gray-300 font-semibold" style={{ color: primaryColor }}>
        ...
        <div className="mb-8">
          <h3 className="text-lg font-semibold mb-2" style={{ color: primaryColor }}>Note</h3>
